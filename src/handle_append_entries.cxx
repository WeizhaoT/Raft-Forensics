...
bool raft_server::request_append_entries(ptr<peer> p) {
    ...
    if (params->use_bg_thread_for_snapshot_io_) {
        ...
        if (snapshot_io_mgr::instance().has_pending_request(this, p->get_id())) {
            p_tr("previous snapshot request for peer %d already exists", p->get_id());
            return true;
        }
    }
    ...
    if (p->make_busy()) {
        ...
        if (msg) {
            ...
            p_in("found reserved message to peer %d, type %d", p->get_id(), msg->get_type());
        ...
        if (!msg) {
            ...
            p->set_free();
            if (params->use_bg_thread_for_snapshot_io_ && p->get_snapshot_sync_ctx()) {
        ...
        if (srv_to_leave_ && srv_to_leave_->get_id() == p->get_id() && msg->get_commit_idx() >= srv_to_leave_target_idx_
...
ptr<req_msg> raft_server::create_append_entries_req(ptr<peer>& pp) {
    ...
    if (last_log_idx >= cur_nxt_idx) {
        // LCOV_EXCL_START
        p_er("Peer's lastLogIndex is too large %llu v.s. %llu, ", last_log_idx, cur_nxt_idx);
    ...
    if (last_log_idx + 1 == peer_last_sent_idx && last_log_idx + 2 < end_idx) {
        ...
        p_db("last sent log (%zu) to peer %d is not applied, cnt %d", peer_last_sent_idx, p.get_id(), cur_cnt);
        if (cur_cnt >= 5) {
    ...
    if (!entries_valid) {
        ...
        if (snp_local && last_log_idx < starting_idx && last_log_idx < snp_local->get_last_log_idx()) {
            ...
            return create_sync_snapshot_req(pp, last_log_idx, term, commit_idx, succeeded_out);
        }
        ...
        ptr<req_msg> req =
            cs_new<req_msg>(term, msg_type::custom_notification_request, id_, p.get_id(), 0, last_log_idx, commit_idx);
    ...
    if (log_entries) {
        //! FORENSICS: RN: use the first log entry to transmit hash pointer
        if (flag_use_ptr() && hash_ptr_buf != nullptr) {
            ptr<log_entry> ptr_msg_le = cs_new<log_entry>(0, hash_ptr_buf, log_val_type::hash_ptr);
            v.push_back(ptr_msg_le);
            p_in("hash pointer (%s) sent to peer %d", tobase64(*hash_ptr_buf).c_str(), p.get_id());
        }
    }

    //! FORENSICS: RN: new leader sig
    if (log_entries && flag_use_leader_sig()) {
        //! FORENSICS: RN: shared the leader signatures of the last shared log entry
        ...
        while (entry_to_sign != log_entries->rend()) {
            if ((*entry_to_sign)->get_val_type() == log_val_type::app_log) {
                ...
                p_in("leader signature on log %zu sent to peer %d",
                     adjusted_end_idx - (log_entries->rend() - entry_to_sign),
                     p.get_id());
    ...
    //! FORENSICS: retransmit old lc and cc
...
ptr<resp_msg> raft_server::handle_append_entries(req_msg& req) {
    ...
    struct ServingReq {
        ServingReq(std::atomic<bool>* _val)
            : val(_val) {
            val->store(true);
        }
    ...
    if (req.get_term() < state_->get_term() || log_okay == false) {
        p_lv(log_lv,
             ...
             log_store_->next_slot() - 1);
        if (local_snp) {
            p_lv(log_lv, "snp idx %zu term %zu", local_snp->get_last_log_idx(), local_snp->get_last_log_term());
        }
    ...
    if (req.log_entries().size() > 0 && req.log_entries().at(0)->get_val_type() == log_val_type::hash_ptr) {
        hash_ptr = buffer::clone(*req.log_entries().at(0)->get_buf_ptr());
        p_in("hash pointer (%s) received from peer %d", tobase64(*hash_ptr).c_str(), req.get_src());
    ...
    if (req.log_entries().size() > 0 && req.log_entries().at(0)->get_val_type() == log_val_type::leader_sig) {
        ...
        while (entry_to_sign != req.log_entries().rend()) {
            if (entry_to_sign->get()->get_val_type() == log_val_type::app_log) {
                ...
                last_app_log_idx =
                    req.get_last_log_idx() + req.log_entries().size() - (req.log_entries().rbegin() - entry_to_sign);
                break;
            }
            // Process the entry and then move to the previous one
            ++entry_to_sign;
        ...
        while (type_ == log_val_type::old_lc || type_ == log_val_type::old_cc) {
            ...
            if (type_ == log_val_type::old_lc) {
                try {
                    ...
                    buf->pos(0);
                    p_db("[LC] received LC of term %zu", lc->get_term());
                    // save lc
                ...
                try {
                    ...
                    buf->pos(0);
                    p_db("[CC] received CC of term %zu and index %zu", cc->get_term(), cc->get_index());
                    // save cc
    ...
    //! FORENSICS: check chain nature and leader sig
    ...
    if (req.log_entries().size() > 0) {
        ...
        while (log_idx < log_store_->next_slot() && cnt < req.log_entries().size()) {
            if (log_store_->term_at(log_idx) == req.log_entries().at(cnt)->get_term()) {
                log_idx++;
                cnt++;
            } else {
                break;
        ...
        if (my_last_log_idx >= log_idx && cnt < req.log_entries().size()) {
            ...
            if (quick_commit_index_ >= log_idx) {
                p_wn("rollback quick commit index from %zu to %zu", quick_commit_index_.load(), log_idx - 1);
            ...
            for (uint64_t ii = 0; ii < my_last_log_idx - log_idx + 1; ++ii) {
                ...
                if (old_entry->get_val_type() == log_val_type::app_log) {
                    ...
                    p_in("revert from a prev config change to config at %llu", get_config()->get_log_idx());
        ...
        if (rollback_in_progress) {
            p_in("last log index after rollback and overwrite: %zu", log_store_->next_slot() - 1);
        }
        ...
        while (cnt < req.log_entries().size()) {
            ...
            if (entry->get_val_type() == log_val_type::conf) {
                p_in("receive a config change from leader at %llu", idx_for_entry);
                config_changing_ = true;
        ...
        if (params->parallel_log_appending_) {
            ...
            while (last_durable_index < req.get_last_log_idx() + req.log_entries().size()) {
                ...
                p_tr("durable idnex %lu, sleep and wait for log appending completion", last_durable_index);
...
bool raft_server::try_update_precommit_index(ulong desired, const size_t MAX_ATTEMPTS) {
    ...
    while (prev_precommit_index < desired && (num_attempts < MAX_ATTEMPTS || MAX_ATTEMPTS == 0)) {
        if (precommit_index_.compare_exchange_strong(prev_precommit_index, desired)) {
            return true;
        }
...
uint64_t raft_server::get_current_leader_index() {
    ...
    if (params->parallel_log_appending_) {
        ...
        p_tr("last durable index %lu, precommit index %lu", durable_index, precommit_index_.load());
...
ulong raft_server::get_expected_committed_log_idx() {
    //! FORENSICS: change the rules for CC usage
    ...
    if (aci_params.expected_commit_index_ != adjusted_commit_index) {
        p_tr("commit index adjusted: %lu -> %lu", aci_params.expected_commit_index_, adjusted_commit_index);
    }
