...
#define __size_of_block(p) \
    (__is_big_block(p)) ? (*((uint*)(p)) ^ 0x80000000) : *((ushort*)(p))
...
#define __entire_data_of_block(p)                            \
    (__is_big_block(p)) ? (byte*)((byte*)(((uint*)(p)) + 2)) \
                        : (byte*)((byte*)(((ushort*)(p)) + 2))

namespace nuraft {

static void free_buffer(buffer* buf) {
    static stat_elem& num_active =
        *stat_mgr::get_instance()->create_stat(stat_elem::COUNTER, "num_active_buffers");
...
ptr<buffer> buffer::alloc(const size_t size) {
    static stat_elem& num_allocs =
        *stat_mgr::get_instance()->create_stat(stat_elem::COUNTER, "num_buffer_allocs");
...
size_t buffer::container_size() const {
    return (size_t)(__size_of_block(this)
                    + ((__is_big_block(this)) ? sizeof(uint) * 2 : sizeof(ushort) * 2));
}
