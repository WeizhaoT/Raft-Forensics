...
bool raft_server::commit_in_bg_exec(size_t timeout_ms) {
    ...
    struct ExecCommitAutoCleaner {
        ExecCommitAutoCleaner(std::function<void()> func)
            : clean_func_(func) {}
...
void raft_server::commit_conf(ulong idx_to_commit, ptr<log_entry>& le) {
    ...
    if (cur_conf->get_log_idx() < new_conf->get_log_idx()) {
        ...
        p_in("skipped config %lu, latest config %lu", new_conf->get_log_idx(), cur_conf->get_log_idx());
    }
...
bool raft_server::snapshot_and_compact(ulong committed_idx, bool forced_creation) {
    ...
    if (!forced_creation) {
        ...
        if (params->snapshot_distance_ == 0 || (committed_idx - log_store_->start_index() + 1) < snapshot_distance) {
    ...
    try {
        ...
        if ((forced_creation || !local_snp || (committed_idx - local_snp->get_last_log_idx()) >= snapshot_distance)
        ...
        p_er("failed to compact logs at index %llu due to errors %s", committed_idx, e.what());
...
void raft_server::reconfigure(const ptr<cluster_config>& new_config) {
    ptr<cluster_config> cur_config = get_config();
    p_in("new config log idx %zu, prev log idx %zu, cur config log idx %zu, prev log idx %zu",
         new_config->get_log_idx(),
         new_config->get_prev_log_idx(),
         cur_config->get_log_idx(),
         cur_config->get_prev_log_idx());
    p_db("system is reconfigured to have %d servers, last config index: %llu, this config index: %llu",
    ...
    for (std::list<ptr<srv_config>>::const_iterator it = new_srvs.begin(); it != new_srvs.end(); ++it) {
        ...
        if (id_ == (*it)->get_id()) {
            ...
            if (role_ == srv_role::follower && catching_up_) {
                ...
                p_in("now this node is the part of cluster, catch-up process is done, clearing the flag");
                catching_up_ = false;
    ...
    for (std::vector<int32>::const_iterator it = srvs_removed.begin(); it != srvs_removed.end(); ++it) {
        ...
        if (srv_removed == id_ && !catching_up_) {
            p_in("this server (%d) has been removed from the cluster, will step down itself soon. config log idx %zu",
                 id_,
                 new_config->get_log_idx());
        ...
        if (pit != peers_.end()) {
            ...
            if (role_ == srv_role::leader && srv_to_leave_) {
                ...
                if (snp_ctx) {
                    ...
                    p_in("srv_to_leave_ has snapshot context %p and user context %p, destroy them",
                         snp_ctx.get(),
                         user_ctx);
                ...
                if (!srv_to_leave_) {
                    p_in("srv_to_leave_ is currently empty on config for removing %d", pp->get_id());
                }
...
void raft_server::pause_state_machine_exeuction(size_t timeout_ms) {
    p_in("pause state machine execution, previously %s, state machine %s, timeout %zu ms",
    ...
    p_in("waited %zu ms, state machine %s", timer.get_ms(), sm_commit_exec_in_progress_ ? "RUNNING" : "SLEEPING");
}
